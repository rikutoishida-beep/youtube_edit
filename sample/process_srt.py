...
def smart_merge_blocks(blocks_in_segment):
    ...
    Also tracks original blocks and character positions for accurate timecode mapping."""
    ...
    current_blocks = [blocks_in_segment[0]]
...
def _build_char_time_map(blocks):
    """Build a character-to-millisecond time map from original blocks."""
    char_map = []
    for block in blocks:
        text = block['text']
        text_len = len(text)
        ...
        time_per_char = block_duration / text_len if text_len > 0 else 0
        for j in range(text_len):
            char_time = int(block_start + j * time_per_char)
            char_map.append(char_time)
    return char_map

def distribute_timecodes(segments_text, total_start, total_end, original_blocks=None, min_duration=1000):
    """Distribute timecodes using original block timing information when available."""
    ...
    char_to_time_map = None
    if original_blocks:
        char_to_time_map = _build_char_time_map(original_blocks)
    ...
    for i, text in enumerate(segments_text):
        ...
        if char_to_time_map and char_pos < len(char_to_time_map):
            start_time = char_to_time_map[char_pos]
            end_pos = min(char_pos + text_len - 1, len(char_to_time_map) - 1)
            end_time = char_to_time_map[end_pos] if end_pos >= 0 else total_end
        else:
            char_ratio = text_len / total_chars if total_chars > 0 else 0
        ...
        if end_time - start_time < min_duration and len(segments_text) > 1:
            end_time = start_time + min_duration

        if i == len(segments_text) - 1:
            end_time = total_end
        elif end_time > total_end:
            end_time = total_end
